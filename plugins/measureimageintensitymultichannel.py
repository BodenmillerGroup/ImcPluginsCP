...
objects.""",
    ...
    def run(self, workspace):
        ...
        # Then measure each
        #
        col_labels = ["Image", "Channel", "Masking object", "Feature", "Value"]
        statistics = []
    ...
    def measure(self, im, workspace):
        ...
        image = workspace.image_set.get_image(
            im.image_name.value, must_be_grayscale=False
        )
        nchan = im.nchannels.value
        statistics = []
        for channel in range(nchan):
            if nchan == 1:
                pixels = image.pixel_data.squeeze()
            else:
                pixels = image.pixel_data[:, :, channel].squeeze()
            ...
            if pixel_count == 0:
                ...
                m.add_image_measurement(
                    F_MEDIAN_INTENSITY % measurement_name, pixel_median
                )
                ...
                m.add_image_measurement(
                    F_PERCENT_MAXIMAL % measurement_name, pixel_pct_max
                )
                m.add_image_measurement(
                    F_LOWER_QUARTILE % measurement_name, pixel_lower_qrt
                )
                m.add_image_measurement(
                    F_UPPER_QUARTILE % measurement_name, pixel_upper_qrt
                )
                statistics += [
                    [
                        im.image_name.value,
    ...
    def upgrade_settings(
        ...
        """
        return setting_values, variable_revision_numbe
