...
class TransformBinary(cpm.Module):
    ...
    def visible_settings(self):
        result = [self.image_name, self.transformed_image_name, self.transform_method]
        return result
    ...
    def run_per_layer(self, image, channel):
        ...
        if self.transform_method.value == DISTANCE_BORDER:
            ...
            raise ValueError(
                "Unsupported transformation method: %s" % self.transform_method.value
            )
    ...
    def run(self, workspace):
        ...
        workspace.image_set.add(self.transformed_image_name.value, output_image)
        workspace.display_data.pixel_data = image.pixel_data
        workspace.display_data.output_pixels = output_pixels

    def display(self, workspace, figure):
        figure.set_subplots((2, 1))
        ax1 = figure.subplot_imshow_color(
            0,
            0,
            workspace.display_data.pixel_data,
            "Original: %s" % self.image_name.value,
        )
        ax2 = figure.subplot_imshow_color(
            1,
            0,
            workspace.display_data.output_pixels,
            "Filtered: %s" % self.transformed_image_name.value,
